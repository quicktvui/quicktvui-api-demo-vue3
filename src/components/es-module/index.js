import es_module_page from './es-module-page';
import es_develop_module_page from './es-develop-module-page';
import es_device_module_page from './es-device-module-page';
import es_toast_module_page from './es-toast-module-page';
import es_app_module_page from './es-app-module-page';
import es_network_module_page from './es-network-module-page';
import es_local_storage_module_page from './es-local-storage-module-page';
import es_power_module_page from './es-power-module-page';
import es_permission_module_page from './es-permission-module-page';
import es_broadcast_module_page from './es-broadcast-module-page';
import es_iac_module_page from './es-iac-module-page';
import es_download_module_page from './es-download-module-page';
import es_upload_module_page from './es-upload-module-page';
import es_audio_module_page from './es-audio-module-page';
import es_file_module_page from './es-file-module-page';
import es_usb_module_page from './es-usb-module-page';
import es_shared_data_module_page from './es-shared-data-module-page';
import es_runtime_module_page from './es-runtime-module-page';
import es_plugin_module_page from './es-plugin-module-page';
import es_actionbar_module_page from './es-actionbar-module-page';
import es_location_module_page from './es-location-module-page';
import es_provider_module_page from './es-provider-module-page';
import es_brightness_module_page from './es-brightness-module-page';
import es_pay_module_page from './es-pay-module-page';
import es_display_module_page from './es-display-module-page';
import es_service_module_page from './es-service-module-page';

const ESModulePageList = {
  es_module_page: {
    name: 'ES模块',
    component: es_module_page,
  },

  es_develop_module_page: {
    name: '开发信息模块',
    component: es_develop_module_page,
  },

  es_device_module_page: {
    name: '设备信息模块',
    component: es_device_module_page,
  },

  es_toast_module_page: {
    name: 'Toast模块',
    component: es_toast_module_page,
  },

  es_app_module_page: {
    name: '应用列表模块',
    component: es_app_module_page,
  },

  es_network_module_page: {
    name: '网络模块',
    component: es_network_module_page,
  },

  es_local_storage_module_page: {
    name: '本地存储',
    component: es_local_storage_module_page,
  },

  es_power_module_page: {
    name: '电源模块',
    component: es_power_module_page,
  },

  es_permission_module_page: {
    name: '权限模块',
    component: es_permission_module_page,
  },

  es_broadcast_module_page: {
    name: '广播模块',
    component: es_broadcast_module_page,
  },

  es_iac_module_page: {
    name: '应用间通信模块',
    component: es_iac_module_page,
  },

  es_download_module_page: {
    name: '下载模块',
    component: es_download_module_page,
  },

  es_upload_module_page: {
    name: '上传模块',
    component: es_upload_module_page,
  },

  es_audio_module_page: {
    name: '音量模块',
    component: es_audio_module_page,
  },

  es_file_module_page: {
    name: '文件模块',
    component: es_file_module_page,
  },

  es_usb_module_page: {
    name: 'Usb设备模块',
    component: es_usb_module_page,
  },

  es_shared_data_module_page: {
    name: '共享数据',
    component: es_shared_data_module_page,
  },

  es_runtime_module_page: {
    name: '运行容器信息',
    component: es_runtime_module_page,
  },

  es_plugin_module_page: {
    name: '插件',
    component: es_plugin_module_page,
  },

  es_actionbar_module_page: {
    name: '顶部菜单条',
    component: es_actionbar_module_page,
  },

  es_location_module_page: {
    name: '定位模块',
    component: es_location_module_page,
  },

  es_provider_module_page: {
    name: 'ContentProvider模块',
    component: es_provider_module_page,
  },

  es_brightness_module_page: {
    name: '亮度调节模块',
    component: es_brightness_module_page,
  },

  es_pay_module_page: {
    name: '支付模块',
    component: es_pay_module_page,
  },

  es_display_module_page: {
    name: '显示模块',
    component: es_display_module_page,
  },

  es_service_module_page: {
    name: '服务模块',
    component: es_service_module_page,
  },
};

export default ESModulePageList;
