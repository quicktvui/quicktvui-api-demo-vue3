...
import {ESBroadcastReceiver, ESIntentFilter, useESBroadcast, useESToast} from "@extscreen/es3-core";
import {ref} from "vue";
...
export default defineComponent({
  ...
  setup() {
    ...
    function onRegisterReceiverAButtonClicked() {
      broadcast.registerReceiver(broadcastReceiverA, receiverAIntentFilter)
        .then((() => {
    ...
    function onRegisterReceiverBButtonClicked() {
      broadcast.registerReceiver(broadcastReceiverB, receiverBIntentFilter)
        .then((() => {
            toast.showToast("注册广播成功！")
            if (log.isLoggable(ESLogLevel.DEBUG)) {
    ...
    function onRegisterReceiverCButtonClicked() {
      //传null,接收不到广播
      broadcast.registerReceiver(broadcastReceiverC, receiverCIntentFilter)
        .then((() => {
            toast.showToast("注册广播成功！")
            if (log.isLoggable(ESLogLevel.DEBUG)) {
    ...
    function onRegisterReceiverDButtonClicked() {
      broadcast.registerReceiver(broadcastReceiverD, receiverDIntentFilter)
        .then((() => {
            toast.showToast("注册广播成功！")
            if (log.isLoggable(ESLogLevel.DEBUG)) {
    ...
    function onUnregisterReceiverAButtonClicked() {
      broadcast.unregisterReceiver(broadcastReceiverA, receiverAIntentFilter)
        .then((() => {
    ...
    function onUnregisterReceiverBButtonClicked() {
      broadcast.unregisterReceiver(broadcastReceiverB, receiverBIntentFilter)
        .then((() => {
            intentText.value = ''
            toast.showToast("取消注册广播成功！")
            if (log.isLoggable(ESLogLevel.DEBUG)) {
    ...
    function onUnregisterReceiverCButtonClicked() {
      broadcast.unregisterReceiver(broadcastReceiverC, receiverCIntentFilter)
        .then((() => {
            intentText.value = ''
            toast.showToast("取消注册广播成功！")
            if (log.isLoggable(ESLogLevel.DEBUG)) {
    ...
    function onUnregisterReceiverDButtonClicked() {
      broadcast.unregisterReceiver(broadcastReceiverD, receiverDIntentFilter)
        .then((() => {
            intentText.value = ''
            toast.showToast("取消注册广播成功！")
            if (log.isLoggable(ESLogLevel.DEBUG)) {
