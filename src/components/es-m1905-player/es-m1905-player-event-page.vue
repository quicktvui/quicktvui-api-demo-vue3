...
import {ESM1905Player} from "@extscreen/es3-m1905-player";

const TAG = 'ESM1905PlayerPage'
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    function onPlayerPlayModeListChanged(modeList: Array<ESPlayerPlayMode>): void {
      if (log.isLoggable(ESLogLevel.DEBUG)) {
        log.d(TAG, '----------onPlayerPlayModeListChanged----->>>', modeList)
      }
    }

    function onPlayerPlayModeChanged(mode: ESPlayerPlayMode): void {
      if (log.isLoggable(ESLogLevel.DEBUG)) {
        log.d(TAG, '----------onPlayerPlayModeChanged----->>>', mode)
      }
    }
