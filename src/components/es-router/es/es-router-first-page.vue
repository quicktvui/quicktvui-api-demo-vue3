...
export default defineComponent({
  ...
  beforeRouteEnter(to, from, next) {
    ...
    // 因为当守卫执行前，组件实例还没被创建
    next()
    console.log('---beforeRouteEnter---第一个页面--->>>>', to, from)
  },
  beforeRouteUpdate(to, from, next) {
    // 以动态路由的方式（如`/foo/:id`），再次访问到当前组件时，调用beforeRouteUpdate
    // 可以访问组件实例 `this`
    console.log('---beforeRouteUpdate---第一个页面--->>>>', to, from)
    next();
  },

  setup() {
    const routerRootView = ref(null)
    const routeLifecycleText = ref(null)
